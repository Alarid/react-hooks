...
`React.useEffect` with no dependencies provided is executed after React has
rendered the component in the DOM, each and every time. When we provide
dependencies, the hook is called every time one of the dependencies changes.

`React.useState` can accept a function instead of the initial value. This
function will only be called the first time the component is render.
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Hooks%20%F0%9F%8E%A3&e=02%3A%20useEffect%3A%20persistent%20state&em=yoh.legrand%40gmail.com).
