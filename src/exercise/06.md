...
It's good practice to avoid defining async function inside the `React.useEffect`
hook. Best to call an async function defined in a util file, and chain the
promise for handling response.

Using `.catch` on promise will not only catch fetching errors, but also errors
that may occur in the success callback.

It's better to rely on a status rather than many boolean checks to manage
different states for a component

As `React.useState` triggers a re-render, the order of the useStates matters,
and can cause problems if updating multiple step in the wrong order. In this
case it's better to store all those values as an object in the state. That is
what `React.useReducer` is made for.

Error boundaries component are really usefull to handle all kind of errors that
may occur in our application. It's not recommended to wrap **everything** in the
error boundary component, but rather place it a various strategic places in our
code base and make it accept a fallback component as prop, to render the error
depending on the context.

A "relatively ok" trick to unmount an error boundary component after an error
has occured is to provide him with a key that we know will change when we want
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Hooks%20%F0%9F%8E%A3&e=06%3A%20useEffect%3A%20HTTP%20requests&em=yoh.legrand%40gmail.com).
